...
###############################################################################
# greaterOrEqual() makes sure the user enters a number greater than or equal
#                  to the minimum value
# Input:           prompt; string to use when asking for an input
#                  minn; minimum value for the number
# Output:          userVal; a number greater than or equal to the minimum
def greaterOrEqual(prompt , minn):
    # prompt the user to enter a number
    userVal = int(input(prompt))

    invalidMsg = "\tThat is not a valid value; please enter a number"
    invalidMsg += "\n\tgreater than or equal to " + str(minn)

    # if the number entered is less than the minimum, reprompt
    while userVal < minn:
        if userVal < minn:
            print(invalidMsg)
            userVal = int(input(prompt))
...
def getValidInput(prompt ,  minn , maxx):
    # prompt user to enter a value
    userVal = input(prompt)

    invalidMsg = "\tThat is not a valid value; please enter a number"
    invalidMsg += "\n\tbetween " + str(minn) + " and " + str(maxx)
    invalidMsg += " inclusive..."
...
# Output:      boardList; 2D list with lengths of rowNum and colNum
...
# Input:      numRows; number of rows in board
#             numCols; number of columns in board
#             board; 2D list of "Dead" cells
# Output:     board; 2D list with "Live" cells based on user input
def makeBoard(numRows , numCols , board):
    ...
    while row != QUIT:
        row = getValidInput(promptRow, 0 , numRows - 1)
        
        # get the column value
        if row != QUIT: 
            col = getValidInput(promptCol, 0 , numCols - 1)
            board[row][col] = ALIVE
...
def main():
    ...
    numRows = greaterOrEqual(promptRow, ROW_MIN)
    numCols = greaterOrEqual(promptCol , COL_MIN)
    ...
    iterNum = greaterOrEqual(promptIter, ITER_MIN)
