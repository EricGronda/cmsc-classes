...
Author: CMSC 202
Date: 10/9/2017
Description: This is introducing deletion and cleanup of linked lists in C++.
*/
...
public:
  ...
  ~LinkedList(); //Destructor to clean up linked list
  void insert_begin(); //Function that inserts new nodes at the beginning of the linked list
  void delete_end(); //Function to delete the last node in a linked list
...
void LinkedList::insert_begin(){
  ...
  if(m_head == NULL){ //Checks to see if the linked list is empty
    ...
    temp->info = m_size+1; //Sets the value of the new node
...
void LinkedList::delete_end(){
  if(m_size==0){
    ...
    Node *nextToEnd = m_head;
    while (nextToEnd->next != m_tail ) {
      cout << nextToEnd->info << endl;
      nextToEnd = nextToEnd->next;
    }
    nextToEnd->next = NULL; //SEG FAULT!!! WHY?
    //    delete m_tail;
    m_tail = nextToEnd;
  }
}
...
int main () {
  ...
  list1.insert_begin(); //Inserts a node
  list1.insert_begin(); //Inserts a node
  list1.delete_end(); //Deletes a node
  list1.display(); //Outputs the linked list (should have three nodes each with a value of 1)
  return 0;
}
